...
const scrypt = async (password, salt, opts) => {
	const { N, r, p, dkLen, blockSize32, V, B32, B, tmp, blockMixCb, asyncTick } = await scryptInit(
		password,
		salt,
		opts
	);
...
const log = (type, text, subtext) => {
	...
	console.log(
		`${dim(/* @__PURE__ */ new Date().toLocaleTimeString())}  ${type} ${text} ${dim(subtext)}`
	);
};
class AuthRequest {
	auth;
	...
	getDatabaseSessionAndUser = async (sessionId) => {
		if (this.adapter.getSessionAndUser) {
			...
			if (!isValidDatabaseSession(databaseSession2)) {
				debug.session.fail(
					`Session expired at ${new Date(Number(databaseSession2.idle_expires))}`,
					sessionId
				);
				throw new LuciaError('AUTH_INVALID_SESSION_ID');
			}
...
export { LuciaError as L, auth as a };
