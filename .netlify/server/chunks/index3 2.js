import {
	t as set_current_component,
	r as run_all,
	u as current_component,
	c as create_ssr_component,
	e as escape,
	a as add_attribute,
	w as add_styles,
	f as get_store_value,
	o as onDestroy
} from './ssr.js';
...
const ProgressBar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return ` <div class="${
		'progress-bar ' + escape(classesTrack, true) + ' svelte-meqa4r'
	}" data-testid="progress-bar" role="progressbar"${add_attribute(
		'aria-labelledby',
		labelledby,
		0
	)}${add_attribute('aria-valuenow', value, 0)}${add_attribute(
		'aria-valuemin',
		min,
		0
	)}${add_attribute('aria-valuemax', max - min, 0)}> <div class="${
		'progress-bar-meter ' +
		escape(classesMeter, true) +
		' ' +
		escape(classesMeter, true) +
		' svelte-meqa4r'
	}"${add_styles({
...
async function _validateField(path, validators, data, Errors, Tainted, options = {}) {
	if (options.update === void 0) options.update = true;
	if (options.taint === void 0) options.taint = false;
	...
	function Tainted_isPathTainted(path2, tainted) {
		if (tainted === void 0) return false;
	...
	function Errors_clearFormLevelErrors() {
		Errors.update(($errors) => {
			traversePaths($errors, (path2) => {
				if (path2.key == '_errors') return path2.set(void 0);
			});
			return $errors;
		});
	}
	...
	return enhance(formEl, async (submit) => {
		...
		if (cancelled) {
			...
			const validation = await clientValidation(
				options,
				get_store_value(data),
				get_store_value(formId),
				get_store_value(constraints),
				get_store_value(posted)
			);
			if (!validation.valid) {
				cancel();
				const result = {
		...
		async function validationResponse(event) {
			...
			if (!cancelled2) {
				if ((result.type === 'success' || result.type == 'failure') && result.data) {
					...
					for (const newForm of forms) {
						if (newForm.id !== get_store_value(formId)) continue;
						const data3 = {
							form: newForm,
							formEl,
							cancel: () => (cancelled2 = true)
					...
					if (result.type !== 'error') {
						...
						if (options.onError !== 'apply') {
							...
							for (const event2 of formEvents.onError) {
								if (event2 !== 'apply') await event2(data3);
							}
						}
					}
			...
			if (cancelled2 || result.type != 'redirect') {
				...
				new URL(
					result.location,
					/^https?:\/\//.test(result.location) ? void 0 : document.location.origin
...
function superForm(form, options = {}) {
	...
	if (!form || Context_isValidationObject(form) === false) {
		if (options.warnings?.noValidationAndConstraints !== false) {
			console.warn(
				(form
					...
					'Also, no constraints will exist for the form. Set the warnings.noValidationAndConstraints option to false to disable this warning.'
			);
		}
		form = {
			valid: false,
		...
		if (_formId === void 0) _formId = form.id;
	}
	...
	if (typeof initialForm.valid !== 'boolean') {
		throw new SuperFormError(
			"A non-validation object was passed to superForm. Check what's passed to its first parameter."
		);
	}
	...
	function Context_useEnhanceEnabled() {
		...
		if (_formId === void 0) FormId.set(Context_randomId());
	}
	...
	const Unsubscriptions = [FormId.subscribe((id) => (_formId = id))];
	...
	async function Tainted_update(newObj, compareAgainst, taintOptions) {
		...
		if (typeof taintOptions === 'object') {
			if (typeof taintOptions.fields === 'string') taintOptions.fields = [taintOptions.fields];
		...
		if (paths.length) {
			...
			if (!updated) await validateObjectErrors(options, get_store_value(Form2), Errors);
		}
	}
	...
	return {
		...
		reset: (options2) =>
			Form_reset(
				options2?.keepMessage ? get_store_value(Message) : void 0,
				options2?.data,
				options2?.id
			)
	};
}
export { ProgressBar as P, superForm as s };
