	...
	function parseFormDataEntry(field, value, typeInfo) {
		...
		if (zodType._def.typeName == 'ZodObject') {
			throw new SuperFormError(
				`Object found in form field "${field}". Set the dataType option to "json" and add use:enhance on the client to use nested data structures. More information: https://superforms.rocks/concepts/nested-data`
			);
		}
...
function validateResult(parsed, schemaData, result) {
	...
	if (!parsed.data) {
		...
		if (!result.success) {
			...
			const errors =
				options2.errors !== false ? mapErrors(result.error.format(), entityInfo2.errorShape) : {};
			const zodKeyStatus = unwrappedSchema._def.unknownKeys;
			const data =
				zodKeyStatus == 'passthrough'
					? { ...clone(entityInfo2.defaultEntity), ...partialData }
					: Object.fromEntries(
							schemaKeys.map((key) => [
								key,
...
export { message as m, superValidate as s };
