...
} from './ProgressBar.svelte_svelte_type_style_lang.js';
...
const LightSwitch = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	return `${
		(($$result.head += `<!-- HEAD_svelte-gewkj4_START --><!-- HTML_TAG_START -->${`<script nonce="%sveltekit.nonce%">(${setInitialClassState.toString()})();<\/script>`}<!-- HTML_TAG_END --><!-- HEAD_svelte-gewkj4_END -->`),
		'')
	} <div class="${
	...
	}" tabindex="0"> <div class="${'lightswitch-thumb ' + escape(classesThumb, true)}"> <svg class="${
	...
	return `${
		$toastStore.length
			? ` <div class="${
			  ...
			  }" data-testid="snackbar-wrapper"> <div class="${
					'snackbar ' + escape(classesSnackbar, true)
			  }">${each(filteredToasts, (t, i) => {
					return `<div${add_attribute(
						'role',
						t.hideDismiss ? 'alert' : 'alertdialog',
						0
					)} aria-live="polite"> <div class="${
						'toast ' +
					...
					}" data-testid="toast"><div class="text-base"><!-- HTML_TAG_START -->${
						t.message
					}<!-- HTML_TAG_END --></div> ${
						t.action || !t.hideDismiss
							? `<div class="${'toast-actions ' + escape(cToastActions, true)}">${
									t.action
										? `<button${add_attribute('class', buttonAction, 0)}><!-- HTML_TAG_START -->${
												t.action.label
										  }<!-- HTML_TAG_END --></button>`
										: ``
							  } ${
									!t.hideDismiss
										? `<button${add_attribute(
												...
												buttonDismiss,
												0
										  )} aria-label="Dismiss toast">${escape(buttonDismissLabel)}</button>`
										: ``
							  } </div>`
							: ``
					}</div> </div>`;
...
const userIcon = '/_app/immutable/assets/user2.fbcfe28d.svg';
const UserSettings_svelte_svelte_type_style_lang = '';
const css = {
	code: '.badge-icon.svelte-147zf7w{cursor:pointer}',
	map: null
};
const UserSettings = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	$$result.css.add(css);
	$$unsubscribe_page();
	return `<div class="badge-icon variant-filled-white p-1 w-7 h-7 dark:bg-white svelte-147zf7w" data-svelte-h="svelte-1s02qg7"><img${add_attribute(
		'src',
		userIcon,
		0
	)} alt="user"></div> <div class="card w-80" data-popup="userSettings"><div class="flex flex-col p-4 gap-4"><h3 class="h3" data-svelte-h="svelte-2koira">Current User</h3> <p>Name: ${escape(
		$page.data.name
	)}</p> <p>Email: ${escape($page.data.email)}</p> <p>Role ${escape(
		$page.data.role
	)}</p> <a href="/password-reset" class="underline text-primary-500 hover:text-primary-800" data-svelte-h="svelte-c7ifji">Change Password</a> <button class="btn btn-primary" data-svelte-h="svelte-1r6wit4">Logout</button> </div> </div>`;
});
