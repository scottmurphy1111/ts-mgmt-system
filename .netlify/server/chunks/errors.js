...
function mergePath(path) {
	return path.reduce((acc, next) => {
		if (typeof next === 'number' || !isNaN(parseInt(String(next), 10))) acc += `[${String(next)}]`;
...
function valueOrDefault(value, strict, implicitDefaults, schemaInfo2) {
	...
	if (strict && value !== void 0) return value;
	...
	if (implicitDefaults) {
		...
		if (zodType._def.typeName == 'ZodSet') return /* @__PURE__ */ new Set();
...
function defaultValues(schema) {
	...
	if (!(schema._def.typeName == 'ZodObject')) {
		throw new SuperFormError(
			'Only Zod schema objects can be used with defaultValues. Define the schema with z.object({ ... }) and optionally refine/superRefine/transform at the end.'
		);
	}
...
function constraints(schema, warnings) {
	function constraint(key, zodType, info) {
		...
		if (zodType._def.typeName == 'ZodString') {
			...
			const pattern =
				patterns.length > 0 && patterns[0].kind == 'regex' ? patterns[0].regex.source : void 0;
...
function clearErrors(Errors, options) {
	Errors.update(($errors) => {
		...
		if (options.undefinePath) setPaths($errors, [options.undefinePath], void 0);
		return $errors;
	});
}
export {
