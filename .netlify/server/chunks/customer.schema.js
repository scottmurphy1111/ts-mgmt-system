...
const truckInfoSchema = z.object({
	...
	year: z
		...
		.max(4, { message: 'Year must be less than 4 characters' })
		.regex(/[0-9]{4}/, { message: 'Year must match pattern XXXX' }),
	make: z
		.string({ required_error: 'Make is required' })
		.min(1, { message: 'Make is required' })
		.max(100, { message: 'Make must be less than 100 characters' })
		.trim(),
	model: z
	...
	vin: z
		...
		.max(17, { message: 'Vin must be less than 17 characters' })
		.trim(),
	startMiles: z
		.string({ required_error: 'Start Miles is required' })
...
export { customerPersonalInfoSchema as c, truckInfoSchema as t };
