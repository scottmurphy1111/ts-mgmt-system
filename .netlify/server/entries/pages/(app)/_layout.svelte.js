import { c as create_ssr_component, s as subscribe, b as each, a as add_attribute, v as validate_component, p as missing_component, e as escape } from "../../../chunks/ssr.js";
import { s as storePopup, T as Toast, A as AppShell, a as AppBar, L as LightSwitch, U as UserSettings, l as logo, b as logoDark } from "../../../chunks/ts-logo-dark.js";
...
const Aside = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  $$unsubscribe_page();
  return `<div class="shadow-md w-64 h-full p-4 bg-white dark:bg-surface-900 z-10"><ul class="m-0 p-0 flex flex-col gap-2 list-none h-full">${each(items, (item) => {
    return `<li><a class="${[
      "inline-flex h-8 w-full gap-2 items-center svelte-1jxavhn",
      $page.url.pathname.startsWith(item.link) ? "active" : ""
    ].join(" ").trim()}"${add_attribute("href", item.link, 0)}>${validate_component(item.icon || missing_component, "svelte:component").$$render($$result, {}, {}, {})} <span class="text-style-md title">${escape(item.title)}</span></a> </li>`;
  })} ${$page.data.role === "ADMIN" ? `<li class="mt-auto"><a class="${[
    "inline-flex h-8 w-full gap-2 items-center svelte-1jxavhn",
    $page.url.pathname.startsWith("/admin") ? "active" : ""
  ].join(" ").trim()}" href="/admin">${validate_component(Settings || missing_component, "svelte:component").$$render($$result, {}, {}, {})} <span class="text-style-md title" data-svelte-h="svelte-obqz9h">Admin</span></a></li>` : ``}</ul> </div>`;
});
  ...
  return `${validate_component(Toast, "Toast").$$render($$result, {}, {}, {})} ${validate_component(AppShell, "AppShell").$$render($$result, {}, {}, {
    ...
    header: () => {
      return `${validate_component(AppBar, "AppBar").$$render(
        ...
        {
          trail: () => {
            return `${validate_component(LightSwitch, "LightSwitch").$$render($$result, {}, {}, {})} ${validate_component(UserSettings, "UserSettings").$$render($$result, {}, {}, {})} `;
          },
          lead: () => {
            return `<a class="w-48" href="/" data-svelte-h="svelte-1s8flk5"><img class="block dark:hidden"${add_attribute("src", logo, 0)} alt="ts-logo"> <img class="hidden dark:block"${add_attribute("src", logoDark, 0)} alt="ts-logo-dark"></a> `;
          }
        }
      )} `;
    },
