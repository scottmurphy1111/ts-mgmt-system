import { c as create_ssr_component, a as add_attribute, s as subscribe, h as setContext, v as validate_component, d as each, m as missing_component, e as escape } from "../../../../chunks/ssr.js";
...
const Dialog = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<dialog class="rounded-md shadow-md"${add_attribute("this", dialog, 0)}>${slots.default ? slots.default({}) : ``}</dialog>`;
});
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = ` <div class="flex justify-between items-center mb-4"><form><div class="flex w-1/3 gap-2"><input class="input min-w-[300px] pr-8" type="text" name="search" placeholder="Search by Last Name"${add_attribute("value", $searchStore.search, 0)}> <div class="flex ml-auto align-middle justify-end gap-3 bg-transparent z-10">${$searchStore.search.length > 0 ? `<button type="button">${validate_component(Close || missing_component, "svelte:component").$$render($$result, {}, {}, {})}</button>` : ``} <button type="submit">${validate_component(Search || missing_component, "svelte:component").$$render($$result, {}, {}, {})}</button></div></div></form> <div class="flex gap-2">${$selectedCustomers.length > 0 ? `<button class="btn variant-filled-error" type="button" data-svelte-h="svelte-k1vnjp">Delete Selected</button>` : ``} <button class="btn btn-primary" data-svelte-h="svelte-1iy1fpc">Add New</button></div></div> ${validate_component(Dialog, "Dialog").$$render(
      ...
      {
        default: () => {
          return `<form class="flex flex-col gap-4 mb-4 p-8" method="post" action="?/create"${add_attribute("this", createNewCustomerForm, 0)}>${validate_component(EditCustomer, "EditCustomer").$$render($$result, { resetForm }, {}, {})}</form>`;
        }
      }
    )} <div class="table-container w-full"><table class="table bg-transparent"><thead data-svelte-h="svelte-y9c6ps"><tr class="svelte-1hdxu1t"><th></th> <th>ID</th> <th>First Name</th> <th>Last Name</th> <th>Email</th> <th>Phone</th> <th>Address</th> <th>City</th> <th>State</th> <th>Zip</th> <th>Trucks</th></tr></thead> <tbody>${$searchStore.filtered.length === 0 ? `<p class="p-4" data-svelte-h="svelte-1nc2q1t">No results found</p>` : `${each($searchStore.filtered, (customer) => {
      return `<tr class="svelte-1hdxu1t"><td><input class="checkbox" type="checkbox"></td> <td>${escape(customer.id)}</td> <td>${escape(customer.firstName)}</td> <td>${escape(customer.lastName)}</td> <td>${escape(customer.email)}</td> <td>${escape(customer.phone)}</td> <td>${escape(customer.address)}</td> <td>${escape(customer.city)}</td> <td>${escape(customer.state)}</td> <td>${escape(customer.zip)}</td> <td><!-- HTML_TAG_START -->${processTruckData(customer)}<!-- HTML_TAG_END --></td> </tr>`;
    })}`}</tbody></table> </div>`;
  } while (!$$settled);
