...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	do {
		$$settled = true;
		$$rendered = `<div class="flex justify-between items-center mb-4"><form><div class="flex w-full gap-2"><input class="input min-w-[400px] pr-8" type="text" name="search" placeholder="Search by Last Name / Company Name"${add_attribute(
			'value',
			$searchStore.search,
			0
			...
			{
				default: () => {
					return `<div class="p-8"><h3 class="h3 mb-8" data-svelte-h="svelte-1jcn7al">Add New Customer</h3> <form class="flex flex-col gap-4 mb-4" method="post" action="?/create"${add_attribute(
						'this',
						...
						{},
						{}
					)}  <div class="flex justify-end gap-2"><div class="" data-svelte-h="svelte-134d65p"><button class="btn btn-primary" type="submit">Save</button></div> <div class=""><button class="btn variant-filled-error text-white" type="button" data-svelte-h="svelte-15h6t1i">Cancel</button></div></div> </form></div>`;
				}
			}
		)} <div class="table-container w-full"><table class="table bg-transparent dark:bg-transparent"><thead><tr class="svelte-1hdxu1t"><th><input class="checkbox" type="checkbox" ${
			$selectedCustomers.length === $searchStore.filtered.length && $selectedCustomers.length > 0
				? 'checked'
				: ''
		}></th> <th data-svelte-h="svelte-1tfe9gf">ID</th> <th data-svelte-h="svelte-qwnva7">First Name</th> <th data-svelte-h="svelte-fhlxzj">Last Name</th> <th data-svelte-h="svelte-oz289s">Company Name</th> <th data-svelte-h="svelte-1y73ygm">Email</th> <th data-svelte-h="svelte-1mpo9xu">Phone</th> <th data-svelte-h="svelte-jssg62">Address</th> <th data-svelte-h="svelte-1tyl379">City</th> <th data-svelte-h="svelte-8azjil">State</th> <th data-svelte-h="svelte-qmwtxj">Zip</th> <th data-svelte-h="svelte-1env53i">Trucks</th></tr></thead> <tbody>${
			$searchStore.filtered.length === 0
				...
				: `${each($searchStore.filtered, (customer) => {
						return `<tr class="svelte-1hdxu1t"><td><input class="checkbox" type="checkbox" ${
							$selectedCustomers.includes(customer.id) ? 'checked' : ''
						}></td> <td>${escape(customer.id)}</td> <td>${escape(
							customer.firstName
						)}</td> <td>${escape(customer.lastName)}</td> <td><!-- HTML_TAG_START -->${
							customer.companyName ? customer.companyName : NULL
						}<!-- HTML_TAG_END --></td> <td>${escape(customer.email)}</td> <td>${escape(
							customer.phone
