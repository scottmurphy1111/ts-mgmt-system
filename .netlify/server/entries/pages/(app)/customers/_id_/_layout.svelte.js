...
const TabAnchor = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	} <div class="tab-label">${slots.default ? slots.default({}) : ``}</div></div></a>`;
});
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
	...
	do {
		$$settled = true;
		$$rendered = `<div class="flex flex-col justify-between mb-4"><a class="text-primary-500 hover:text-primary-700" href="/customers" data-svelte-h="svelte-11v3h19">&lt; Back</a> <h3 class="h3">${escape(
			$page.data.customer.firstName
		)} ${escape($page.data.customer.lastName)} - Cust Id# ${escape(
			$page.data.customer.id
