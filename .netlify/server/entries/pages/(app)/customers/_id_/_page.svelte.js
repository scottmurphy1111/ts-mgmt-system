import { c as create_ssr_component, h as setContext, e as escape, a as add_attribute, j as compute_slots, k as compute_rest_props, g as getContext, l as spread, p as escape_attribute_value, q as escape_object, s as subscribe, v as validate_component } from "../../../../../chunks/ssr.js";
...
const DisplayCustomer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="flex gap-4 w-full"><label class="font-light" for="firstName">First Name:
		...
		<p class="text-md font-semibold">${escape($customerFormStore?.data?.firstName)}</p></label> <label class="font-light" for="lastName">Last Name:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.lastName)}</p></label></div> <div class="flex gap-4 w-full"><label class="font-light" for="address">Address:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.address)}</p></label></div> <div class="flex gap-4 w-full"><label class="font-light" for="city">City:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.city)}</p></label> <label class="font-light" for="state">State:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.state)}</p></label> <label class="font-light" for="zip">Zip:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.zip)}</p></label></div> <div class="flex gap-4 w-full"><label class="font-light" for="email">Email:

		<p class="text-md font-semibold">${escape($customerFormStore?.data?.email)}</p></label> <label class="font-light" for="phone">Phone:
...
const PersonalInfo = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_customerFormStore();
  $$unsubscribe_page();
  return `<section class="p-4 w-full"><form class="flex flex-col gap-4 mb-4" method="post" action="?/update"${add_attribute("this", editCustomerForm, 0)}>${$customerFormStore?.status !== "editing" ? `${validate_component(DisplayCustomer, "DisplayCustomer").$$render($$result, {}, {}, {})}` : ``} ${$customerFormStore?.status === "editing" ? `${validate_component(EditCustomer, "EditCustomer").$$render($$result, { resetForm }, {}, {})}` : ``}</form> ${$customerFormStore?.status !== "editing" ? `<div class="flex justify-end gap-2"><button class="btn btn-primary" type="button" data-svelte-h="svelte-1583lpd">Edit</button></div>` : ``}</section>`;
});
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    $$settled = true;
    $$rendered = `<div class="flex justify-between mb-4"><h3 class="h3">${escape($page.data.customerData.firstName)} ${escape($page.data.customerData.lastName)} - Cust Id# ${escape($page.data.customerData.id)}</h3></div> ${validate_component(TabGroup, "TabGroup").$$render($$result, {}, {}, {
      panel: () => {
        return `${tab === 0 ? `${validate_component(PersonalInfo, "PersonalInfo").$$render($$result, { data }, {}, {})}` : `${tab === 1 ? `<h2 data-svelte-h="svelte-14eh9ve">Trucks</h2>` : `${tab === 2 ? `<h2 data-svelte-h="svelte-1lp3ihg">Lender</h2>` : `<h2 data-svelte-h="svelte-r6zq5o">Other</h2>`}`}`} `;
      },
      default: () => {
